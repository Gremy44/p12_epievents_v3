...
class Events_View:
    ...
    def events_id_input(self, length_queryset):
        ...
        id = self.validators.validator_menu_choice(1, length_queryset)
        return id
    ...
    def events_view_list(self, table_name, events, user, queryset_length):
        ...
        for event in events:
            try:
                print(
                    f"ID : {event.id}|Nom de l'évènement: {event.name}|"
                    f"Lieu: {event.location}|Client: {event.client.complet_name}|"
                    f"Contrat: {event.contract.id}|"
                    f"Contacte support: {event.support_contact.first_name}"
                    f"{event.support_contact.last_name}"
                )
            except AttributeError:
                print(
                    f"ID : {event.id}|Nom de l'évènement: {event.name}|"
                    f"Lieu: {event.location}|Client: {event.client.complet_name}|"
                    f"Contrat: {event.contract.id}|Contacte support: Aucun"
                )
        ...
        if user.role_id == 3:
            ...
            for ns in events:
                if ns.support_contact_id == "":
                    print(
                        f"ID : {ns.id}|Nom de l'évènement: {ns.name}|"
                        f"Lieu: {ns.location}|Client: {ns.client.complet_name}|"
                        f"Contrat: {ns.contract.id}"
                    )
        ...
        if user.role_id == 2:
            ...
            for me in events:
                if me.support_contact_id == user.id:
                    print(
                        f"ID : {me.id}|Nom de l'évènement: {me.name}|"
                        f"Lieu: {me.location}|Client: {me.client.complet_name}|"
                        f"Contrat: {me.contract.id}"
                    )
    ...
    def events_view_detail(self, event):
        ...
        print(
            f" - Nom:{event.support_contact.last_name} Prénom :{event.support_contact.first_name}"
        )
        print("----------------------------------------------------------")
        Utils().wait_user_input()
