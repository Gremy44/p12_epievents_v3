...
class Utils:
    ...
    def retrieve_contract(self, client_id):
        ...
        contracts = (
            ...
            .filter(Contract.status == 1, Contract.customer_id == client_id)
            .all()
        )

        for contract in contracts:
            customer_name = (
                contract.customer.complet_name if contract.customer else "N/A"
            )
            company_name = (
                contract.customer.company_name if contract.customer else "N/A"
            )
            print(
                f"ID: {contract.id}||Client associé: {customer_name}|| Compagnie: {company_name}"
            )
